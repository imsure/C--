...
def performance():
    ...
    for line in f.readlines():
        ...
        if testname.endswith( '-O2' ):
            stats_with_O2[ testname ] = ( total, reads, writes, branches, other )
        else:
            stats_without_O2[ testname ] = ( total, reads, writes, branches, other )

    ## Print stats into a table.
    for k in sorted( stats_without_O2 ):
        v = stats_without_O2[ k ]
        print( '{0:14} {1:12} {2:6} {3:6} {4:10} {5:8}'.format(k, 'instructions',
                                                                     'reads', 'writes',
                                                                     'branches', 'other') )
        print( '{0:14} {1:12} {2:6} {3:6} {4:10} {5:8}'.format('without-O2', v[0], v[1],
                                                               v[2], v[3], v[4]) )
        k2 = k + '-O2'
        v2 = stats_with_O2[ k2 ]
        print( '{0:14} {1:12} {2:6} {3:6} {4:10} {5:8}'.format('with-O2', v2[0], v2[1],
                                                               v2[2], v2[3], v2[4]) )
