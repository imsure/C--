...
_L1:	li $8, 1 # Load int constant shorter than 16-bits
	move $11, $8 # Store local tmp _tvar3 to its pre-assigned register.
	...
	move $11, $8 # Store local tmp _tvar5 to its pre-assigned register.
	sw $8, -24($fp) # Store to tmp variable _tvar5.
	lw $8, 8($fp) # Load int formal n.
	sub $10, $8, $11
	move $11, $10 # Store local tmp _tvar5 to its pre-assigned register.
	sw $10, -24($fp) # Store to tmp variable _tvar5.
	la $sp, -4($sp) # Allocate stack space for parameter _tvar5.
	sw $11, 0($sp) # Push the parameter value.
	jal _f
	la $sp, 4($sp) # Pop parameters off the stack.
	move $11, $2 # retrieve return value into _tvar4
	lw $12, -12($fp) # Load local tmp _tvar2 to its pre-assigned register.
	add $10, $12, $11
	move $11, $10 # Store local tmp _tvar4 to its pre-assigned register.
	sw $10, -20($fp) # Store to tmp variable _tvar4.
	move $2, $11 # return int _tvar4
	la $sp, 0($fp) # Move stack pointer back to $fp.
	lw $ra, 0($sp) # Restore return address.
	lw $fp, 4($sp) # Restore frame pointer.
	la $sp, 8($sp) # Restore stack pointer.
...
_L3:	li $8, 1 # Load int constant shorter than 16-bits
	move $11, $8 # Store local tmp _tvar6 to its pre-assigned register.
	sw $8, -8($fp) # Store to tmp variable _tvar6.
	move $2, $11 # return int _tvar6
	la $sp, 0($fp) # Move stack pointer back to $fp.
	lw $ra, 0($sp) # Restore return address.
	lw $fp, 4($sp) # Restore frame pointer.
	la $sp, 8($sp) # Restore stack pointer.
...
_L4:	li $8, 1 # Load int constant shorter than 16-bits
	...
	jal _f
	la $sp, 4($sp) # Pop parameters off the stack.
	move $12, $2 # retrieve return value into _tvar7
	li $8, 2 # Load int constant shorter than 16-bits
	move $11, $8 # Store local tmp _tvar10 to its pre-assigned register.
	sw $8, -24($fp) # Store to tmp variable _tvar10.
	lw $8, 8($fp) # Load int formal n.
	sub $10, $8, $11
	move $11, $10 # Store local tmp _tvar10 to its pre-assigned register.
	sw $10, -24($fp) # Store to tmp variable _tvar10.
	la $sp, -4($sp) # Allocate stack space for parameter _tvar10.
	sw $11, 0($sp) # Push the parameter value.
	jal _g
	la $sp, 4($sp) # Pop parameters off the stack.
	move $11, $2 # retrieve return value into _tvar9
	lw $12, -12($fp) # Load local tmp _tvar7 to its pre-assigned register.
	add $10, $12, $11
	move $11, $10 # Store local tmp _tvar9 to its pre-assigned register.
	sw $10, -20($fp) # Store to tmp variable _tvar9.
	move $2, $11 # return int _tvar9
	la $sp, 0($fp) # Move stack pointer back to $fp.
	lw $ra, 0($sp) # Restore return address.
	lw $fp, 4($sp) # Restore frame pointer.
	la $sp, 8($sp) # Restore stack pointer.
...
main:	la $sp, -8($sp) # Allocate space for old $fp and $ra
	...
	jal _f
	la $sp, 4($sp) # Pop parameters off the stack.
	move $11, $2 # retrieve return value into _tvar10
	move $11, $11 # Store local int x to its pre-assigned register.
	sw $11, -4($fp) # Store to local int x.
