...
# Load constant directly without going through a scratch register.
	li $12, 0 # Load int constant shorter than 16-bits
	j _L1
	.text
_L0:	move $11, $12 # Store local tmp _tvar1 to its pre-assigned register.
	lw $8, 8($fp) # Load address of formal char array b.
	add $10, $8, $11
	sw $10, -24($fp) # Store address to tmp address _taddr2.
	move $11, $12 # Store local tmp _tvar1 to its pre-assigned register.
	...
	.text
_L1:	move $11, $12 # Store local tmp _tvar0 to its pre-assigned register.
	lw $8, 12($fp) # Load address of formal char array a.
	add $10, $8, $11
	...
	bgt $8, $9, _L0
	.text
_L2:	move $11, $12 # Store local tmp _tvar1 to its pre-assigned register.
	lw $8, 8($fp) # Load address of formal char array b.
	add $10, $8, $11
...
# Load constant directly without going through a scratch register.
	li $13, 0 # Load int constant shorter than 16-bits
# Load constant directly without going through a scratch register.
	li $12, 0 # Load int constant shorter than 16-bits
	j _L4
	.text
_L3:	move $11, $13 # Store local tmp _tvar3 to its pre-assigned register.
	lw $8, 8($fp) # Load address of formal char array b.
	add $10, $8, $11
	...
	.text
_L4:	move $11, $13 # Store local tmp _tvar2 to its pre-assigned register.
	lw $8, 8($fp) # Load address of formal char array b.
	add $10, $8, $11
...
main:	la $sp, -8($sp) # Allocate space for old $fp and $ra
	...
	la $sp, -4($sp) # Allocate stack space for parameter x.
	sw $8, 0($sp) # Push the parameter value.
	sw $12, -4($fp) # Store local int/tmp _tvar4 from its pre-assigned register to memory location.
	sw $11, -8($fp) # Store local int/tmp y from its pre-assigned register to memory location.
	jal _sum
	la $sp, 4($sp) # Pop parameters off the stack.
	lw $12, -4($fp) # Reload local int/tmp _tvar4 into its pre-assigned register.
	lw $11, -8($fp) # Reload local int/tmp y into its pre-assigned register.
	move $12, $2 # retrieve return value into _tvar4
	add $10, $11, $12
	move $12, $10 # Store local tmp _tvar4 to its pre-assigned register.
	move $11, $12 # Store local int y to its pre-assigned register.
