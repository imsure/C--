...
main:	la $sp, -8($sp) # Allocate space for old $fp and $ra
	...
	move $15, $8 # Store local int i to its pre-assigned register.
	li $8, 123 # Load int constant shorter than 16-bits
	move $14, $8 # Store local int res to its pre-assigned register.
	j _L1
	.text
_L0:	li $8, 0 # Load int constant shorter than 16-bits
	move $13, $8 # Store local int j to its pre-assigned register.
	j _L4
	.text
_L3:	add $10, $15, $13
	...
	# k was allocated with $11 by the optimizer, but it is wrong because
	# its value will be overwritten by a variable (probably _tvar2 or _tvar3)
	# that also associated with with $11 but have overlap live range with k.
	move $20, $11 # Store local int k to its pre-assigned register.
	ble $20, $15, _L7
	.text
...
_L11:	add $10, $14, $11
	move $11, $10 # Store local tmp _tvar3 to its pre-assigned register.
	move $14, $11 # Store local int res to its pre-assigned register.
	j _L13
	.text
_L12:	add $10, $14, $20 # $11 --> $20(k)
	move $11, $10 # Store local tmp _tvar3 to its pre-assigned register.
	move $14, $11 # Store local int res to its pre-assigned register.
	.text
...
_L7:	li $9, 1 # Load int constant shorter than 16-bits
	add $10, $13, $9
	move $11, $10 # Store local tmp _tvar3 to its pre-assigned register.
	move $13, $11 # Store local int j to its pre-assigned register.
	.text
