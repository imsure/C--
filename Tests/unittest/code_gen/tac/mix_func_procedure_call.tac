...
_L0:	li $8, 1 # Load int constant shorter than 16-bits
	...
	j _L2
	.text
_L1:	lw $8, 8($fp) # Load int formal u.
	...
	lw $2, -12($fp) # Load tmp variable _tvar2.
	la $sp, 0($fp) # Move stack pointer back to $fp.
	lw $ra, 0($sp) # Restore return address.
	lw $fp, 4($sp) # Restore frame pointer.
	la $sp, 8($sp) # Restore stack pointer.
...
incr:
	...
	.data
	.text
_incr:	la $sp, -8($sp) # Allocate space for old $fp and $ra
	sw $fp, 4($sp) # Save old $fp on stack
	sw $ra, 0($sp) # Save old $ra on stack
	la $fp, 0($sp) # Set up frame pointer
	la $sp, -4($sp) # Allocate stack frame for locals/tmps
	lw $8, 8($fp) # Load int formal x.
	lw $9, _u # Load global int u.
	add $10, $8, $9
	sw $10, -4($fp) # Store to tmp variable _tvar4.
...
main:	la $sp, -8($sp) # Allocate space for old $fp and $ra
	...
	li $8, 1 # Load int constant shorter than 16-bits
	sw $8, -4($fp) # Store to tmp variable _tvar4.
	lw $8, -4($fp) # Load tmp variable _tvar4.
	sw $8, _u # Store to global int u.
	...
	lw $9, -12($fp) # Load local int x.
	add $10, $8, $9
	sw $10, -8($fp) # Store to tmp variable _tvar5.
	lw $8, -8($fp) # Load tmp variable _tvar5.
	sw $8, -12($fp) # Store to local int x.
